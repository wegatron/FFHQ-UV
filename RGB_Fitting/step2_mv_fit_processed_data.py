...
if __name__ == '__main__':
    ...
    logger = Logger(
        ...
        is_tb=True)
    logger.write_txt_log(f'Fit image...')
    input_datas = []
    for fn in fnames:
    ...
    tic = time.time()
    fit_model.mv_fitting(input_datas=input_datas, logger=logger)
    toc = time.time()

    logger.write_txt_log(f'Fit image done, took {toc - tic:.4f} seconds.')
    logger.close()
